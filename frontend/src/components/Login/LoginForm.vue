<template>
    <form @submit.prevent="" class="login-form">
        <AppLogo />
        <p class="login-form__title">Войдите или 
            <router-link class="login-form__title-link" to="/registration">зарегистрируйтесь</router-link>
        </p>
        <AppInput 
            v-model="login"
            name="login"
...
async function tryToLogin() {
    ...
    if (isFormValid.value) {
        ...
        accountStore.login(login.value, password.value).then(() => {
                router.push('/');
